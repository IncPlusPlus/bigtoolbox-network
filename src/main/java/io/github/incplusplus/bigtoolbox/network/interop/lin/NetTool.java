  ...
  NetworkManager(
      new Function<String, Boolean>() {
        ...
        public Boolean apply(String s) {
          try (DBusConnection dbusConn =
              DBusConnection.getConnection(DBusConnection.DBusBusType.SYSTEM)) {
            DBusPath nmUnit =
                dbusConn
                    .getRemoteObject(
                        "org.freedesktop.systemd1", "/org/freedesktop/systemd1", Manager.class)
                    .GetUnit("bluetooth.service");
            Properties nmProps =
                dbusConn.getRemoteObject(
                    "org.freedesktop.systemd1", nmUnit.toString(), Properties.class);
            Map<String, Variant<?>> unitProps = nmProps.GetAll("org.freedesktop.systemd1.Unit");
            Map<String, Variant<?>> serviceProps =
                nmProps.GetAll("org.freedesktop.systemd1.Service");
