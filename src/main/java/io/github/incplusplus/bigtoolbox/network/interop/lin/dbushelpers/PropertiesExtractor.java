...
public final class PropertiesExtractor {
	 ...
	 * @return Get all properties from all interfaces this device implements including that of the device itself
	 * @throws IOException
	 */
	public static Map<String, Map<String, Variant<?>>> GetAllFromDevice(Device device) throws IOException {
		Map<String, Map<String, Variant<?>>> out = new HashMap<>();
		Properties deviceProps = getRemoteObject(NM_BUS_PATH,
		...
		for (Class<?> interf : device.getClass().getInterfaces()) {
			...
			for (Class<?> subint : interf.getInterfaces()) {
				if (subint.equals(Properties.class) || subint.equals(DBusInterface.class))
					continue;
