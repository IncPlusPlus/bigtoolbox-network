...
public class NMInterop extends NetworkController {
	...
	public Interface[] getInterfaces() throws IOException {
		try {
			NetworkManager nm = getDbusConn().getRemoteObject("org.freedesktop.NetworkManager",
					"/org/freedesktop/NetworkManager", NetworkManager.class);
			return nm.GetDevices().stream().map(NMDeviceType::dbusInterfaceToNMDevice)
					.map(NMDeviceType::encapsulateNMDeviceInInterface).toArray(
					Interface[]::new);
		}
		catch (DBusException e) {
			throw new IOException(e);
