...
import io.github.incplusplus.bigtoolbox.network.interop.lin.nm.org.freedesktop.networkmanager.device.Generic;
...
public enum NMDeviceType {
  ...
  private static final Map<UInt32, NMDeviceType> NM_DEVICE_TYPE_MAP;
  private static final Map<Class<? extends Device>, NMDeviceType> deviceTypesByDeviceInterface =
      new HashMap<>();

  static {
    NM_DEVICE_TYPE_MAP =
        Arrays.stream(NMDeviceType.values())
            .collect(Collectors.toMap(NMDeviceType::getDeviceType, Function.identity()));
  ...
  public static Device dbusInterfaceToNMDevice(DBusInterface dBusInterface) {
    try {
      ...
      Properties deviceProps =
          dbusConn.getRemoteObject(NM_BUS_PATH, dBusInterface.getObjectPath(), Properties.class);
  ...
  private static Device getDeviceAtPath(NMDeviceType deviceType, String objectPath)
      throws IOException {
    switch (deviceType) {
