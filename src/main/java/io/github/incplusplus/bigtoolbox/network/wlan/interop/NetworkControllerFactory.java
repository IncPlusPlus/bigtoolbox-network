...
/**
 * The {@link NetworkControllerFactory} is a pseudo-singleton pattern factory. The goal of this class is to allow for
 * the creation of {@link NetworkController}s one at a time.
 */
public final class NetworkControllerFactory
{
	...
	/**
	 * Create and return a new NetworkController implementation based on the host's OS.
	 ...
	 * You will not be able to use anything related to this {@link NetworkController} after the try-with-resources
	 * block or after calling {@link NetworkController#close()} and all objects generated using it will also become unusable.
	 * @throws IllegalStateException if the factory is in an unexpected state or if
	 *                         there already is a non-closed NetworkController instance.
	 ...
	 */
	public static NetworkController createNetworkController() throws UnsupportedOSException, IOException {
	...
	private static NetworkController getByOSFamily(OperatingSystem.OSFamily family) throws UnsupportedOSException, IOException {
		switch(family)
		{
			case Windows:
