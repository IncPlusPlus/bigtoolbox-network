...
public interface SecretAgent extends DBusInterface {
   ...
   *
   * @param connection     Nested settings maps containing the connection for which secrets are
   *                       being requested. This may contain system-owned secrets if the agent has
   *                       successfully authenticated to modify system network settings and the
   *                       GetSecrets request flags allow user interaction.
   ...
   * @param hints          Array of strings of key names in the requested setting for which
   *                       NetworkManager thinks a secrets may be required, and/or well-known
   *                       identifiers and data that may be useful to the client in processing the
   *                       secrets request. Note that it's not always possible to determine which
   *                       secret is required, so in some cases no hints may be given. The Agent
   *                       should return any secrets it has, or that it thinks are required,
   *                       regardless of what hints NetworkManager sends in this request. Some hints
   *                       have special prefixes that provide information to the agent; for example,
   *                       VPN requests may send server-specific messages prefixed with
   *                       "x-vpn-message:".
   * @param flags          (NMSecretAgentGetSecretsFlags) Flags which modify the behavior of the
   *                       secrets request. If true, new secrets are assumed to be invalid or
   *                       incorrect, and the agent should ask the user for new secrets. If false,
   *                       existing secrets should be retrieved from storage and returned without
   ...
   *
   * @param connection     Nested settings maps containing the entire connection (including
   *                       secrets), for which the agent should save the secrets to backing storage.
   *                       This method will not be called when the agent itself is the process
   *                       creating or updating a connection; in that case the agent is assumed to
   *                       have already saved those secrets since it had them already.
   * @param connectionPath Object path of the connection for which the agent should save secrets to
   *                       backing storage.
   */
  void SaveSecrets(
      Map<String, Map<String, Variant<?>>> connection, DBusInterface connectionPath);
